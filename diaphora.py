...
class CBinDiff:
  ...
  def create_schema(self):
    ...
    sql = """ create table if not exists reference_labels (
                id integer primary key,
                name text,
                comment1 text,
                comment2 text,
    ...
    sql = "create index if not exists idx_assembly on reference_labels(name)"
    cur.execute(sql)
  ...
  def save_function(self, props):
    ...
    sql = """insert into main.functions (name, nodes, edges, indegree, outdegree, size,
                                    ...
                                    comment, comment_nrpt, mangled_function, bytes_hash, pseudocode,
